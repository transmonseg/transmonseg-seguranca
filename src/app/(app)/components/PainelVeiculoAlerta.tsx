...
export default function PainelVeiculoAlerta({ cv, placa, alertas, onFechar, onAlertaResolvido, empresa }: Props) {
  ...
  function iniciarDrag(e: React.MouseEvent) {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      setPos({
        x: dragRef.current.ix + ev.clientX - dragRef.current.sx,
        y: dragRef.current.iy + ev.clientY - dragRef.current.sy,
      });
    };
    const onUp = () => {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 9, minWidth: 0 }}>
          <span style={{ color: "var(--text-dim)", fontSize: 13, letterSpacing: 1, flexShrink: 0, opacity: 0.55 }}>⠿</span>
          <span
            style={{
